...
% segment
cmpMax = 0;
nCmpGuess=200; equiv = sparse(nCmpGuess,nCmpGuess);	% 'pre-allocate'
...
i
	for j = 1:size(conn,2),
		for k = 1:size(conn,3),
			...
			if connAll(i,j,k)==0,		% no connected neighbors on the 'left'
				% create a new component
				cmpMax = cmpMax + 1;
				equiv(cmpMax,cmpMax) = 1;
				cmp = cmpMax;

			elseif connAll(i,j,k)>0,	% one or more connected neighbors
