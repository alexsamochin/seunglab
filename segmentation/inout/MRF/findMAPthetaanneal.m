function [s, varargout] = findMAPanneal(Jparam,theta,nhood,im,varargin)
% find MAP
...
%    keyboard
    ...
    update=double(reshape(rand(numel(tmpp)/2,1)<p(:,1),pm,pn,pl))*2-1;
...
%    keyboard
    ...
    if (nargin>4)
      pixelperf = 1-(sum(sum(sum(abs(s-varargin{1}))))/2)/numel(varargin{1})
    end
