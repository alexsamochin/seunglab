...
for t=1:T
  t
  tic
  net = neighsvalid*[Jparam Jparam]'+bias;
  sigma=tanh(net);
  sumsigma=sum(sigma);
...
% plus symmetric terms
  sumsisigma=sum(sisigma,1);
  sumsisigma=(sumsisigma(1:nhoodsize/2)+sumsisigma(nhoodsize/2+1:end))/2;
  
%  keyboard
  
  gradJ = (sumsisj(1:nhoodsize/2) - sumsisigma)/nmask
  ...
  montage2(k);
  figure(2);
  sigmashape=reshape(sigma,m,n,l);
  imagesc(sigmashape(:,:,20));
  figure(3);
  imagesc(mtrace(:,:,20));
  
  gradbias = (sumsi - sumsigma)/nmask
  bias=bias + eta*gradbias
  toc
...
mu2=mean(im2);
all=[im1-mu1; im2-mu2];
sigma2 = sum(all.^2)/length(all);

condparam=[mu1  mu2 sqrt(sigma2)];
