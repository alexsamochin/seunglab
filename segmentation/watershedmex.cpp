 ...
 * based on marker-based MAXIMUM spanning tree */
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	// high-threshold to merge upto
    if (!mxIsDouble(prhs[2])){
        mexErrMsgTxt("Conn array must be floats (singles)");
    }
	const double high_threshold = (const double) mxGetScalar(prhs[2]);



	/****** Create the disjoint sets ***************************************************/
	const mwSize num_vertices = 1;
	for (mwSize i=0; i<(conn_num_dims-1); i++){
		num_vertices = num_vertices*conn_dims[i];
	}
