...
if strcmp(whs.class, 'uint8')
    ...
    sem_image = double(sem_image) ./ 256;
else
    sem_image = double(sem_image);
end
...
try 
    ...
    infile = load(valid_filename);
    
    state = infile.state;
    valid_comp = infile.valid_comp;
    valid_info = infile.valid_info;
    problem_area = infile.problem_area;
    ...
    valid_comp = zeros(size(all_comps));
...
% For normal viewing 
h_plot3 = axes('Units', 'Pixels', 'Position', ...
...
set(h_plot3, 'DataAspectRatio', [1 1 zratio]);
...
if strcmp(key,'uparrow')  
    if state.z < sz
        ...
        if state.plot3d_plane
            ...
            set(h_plane,'ZData', surz, 'CData', sem_image(:,:,state.z));
        end
...
elseif(key == 'j')
    ...
    axes(h_plot3); camorbit(6, 0); drawnow;
...
elseif(key == 'i')
    ...
    axes(h_plot3); camorbit(0, 5); drawnow;
...
elseif(key == 'd')
    ...
    axes(h_plot3); camdolly(-.2,0,0); drawnow;
...
elseif(key == 'r')
    ...
    axes(h_plot3); camdolly(0,.2,0); drawnow;
...
elseif(key == 'w')
    ...
    axes(h_plot3); camzoom(1.1); drawnow;

elseif(key == 's')
    % Move 3d plot backwards
    axes(h_plot3); camzoom(0.9); drawnow;
...
if state.plot3d_plane
    ...
    % Gray plane
    %h_plane = patch('Vertices', vertex, 'Faces', faces, 'FaceAlpha', 0.5);

    % Image texture mapped plane
    surz = [state.z state.z ; state.z state.z ];
    sury = [1 sy];
    surx = [1 sx];
    h_plane = surface(surx, sury, surz, 'FaceColor', 'texturemap',  'CData', sem_image(:,:,state.z), 'FaceAlpha', 0.65); 
    colormap(gray(256));
end
% ---- End plane ----
...
if state.thresh == 999
    str = sprintf('Section: %d     Valid components', state.z);
else
    str = sprintf('Section: %d     Threshold: %d', state.z, state.thresh);
end
