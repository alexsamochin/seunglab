...
% Piotr's Image&Video Toolbox      Version 1.02   
% Written and maintained by Piotr Dollar    pdollar-at-cs.ucsd.edu 
% Please email me if you find bugs, or have suggestions or questions! 
 
function C = convn_fast( A, B, shape )
    if( nargin<3 || isempty(shape)) shape='full'; end;
    if(isempty(strmatch(shape, strvcat( 'same', 'valid', 'full', 'smooth'))))
    ...
    if( smoothFlag ) 
        ...
        if( ~all( B>0 ) ) 
            error('If flag==''smooth'' then mask must have strictly positive values.');
        end;            
    ...
    %%% OPTIMIZATION for 3D conv when B is actually 2D - calls (spatial) conv2 repeatedly 
    %%% on 2D slices of A.  Note that may need to rearange A and B first and use
    %%% recursion. The benefits carry over to convn_boundaries (which is faster for 2D
    %%% arrays). 
    ...
    frequen_pt = 3 * 2.1e-8 * psizfft * log(psizfft);
    if (nd==2)
        spatial_pt = 1.8e-9 * sizA(1) * sizA(2) * sizB(1) * sizB(2);
    else 
        spatial_pt = 2.3e-8 * prod(sizA) * prod(sizB);
    end
...
function C = convn_boundaries( A, B, C, sizA, sizB )
    ...
    elseif( nd==3 ) % accelerated for 3D arrays [stupid that can't speed up more]
        for i=1:nlocs
            Apart = A( Astarts(i,1):Aends(i,1), Astarts(i,2):Aends(i,2), ...
                                                    Astarts(i,3):Aends(i,3) );
            Bpart = B( Bstarts(i,1):Bends(i,1), Bstarts(i,2):Bends(i,2), ...
                                                    Bstarts(i,3):Bends(i,3) );
            za = sum(sum(sum(Apart.*Bpart)));  zb=sum(sum(sum(Bpart)));  vs(1,i) = za./zb;
        end
    else % general case [slow]
        for i=1:nlocs
            for d=1:nd extract{d} = Astarts(i,d):Aends(i,d); end; Apart = A( extract{:} );
            for d=1:nd extract{d} = Bstarts(i,d):Bends(i,d); end; Bpart = B( extract{:} );
            v = (Apart.*Bpart); vs(i) = sum(v(:)) ./ sum(Bpart(:));
        end
    end
    C( Dind ) = vs * sum(B(:));
...
function K = time_conv
    mintime = 4; 
    
    switch 3
        case 1  %%% conv2  [[empirically K = 5e-9]] [SCT:blueprint K=2e-9]
            % convolution time = K*prod(size(a))*prod(size(b))
            siza = 30;  sizb = 200;
            a = ones(siza);  b = ones(sizb);  
            t1 = cputime;  t2 = t1;  k = 0;
            while (t2-t1)<mintime c = conv2(a,b); k = k + 1; t2 = cputime; end
            K = (t2-t1)/k/siza^2/sizb^2
        
        case 2  %%% convn  [[empirically K = 5e-8]] [SCT:bluepint K=3e-8]
            % convolution time = K*prod(size(a))*prod(size(b))  
            siza = [10 10 10];  sizb = [30 30 10];
            a = ones(siza);  b = ones(sizb);  
            t1 = cputime;  t2 = t1;  k = 0;
            while (t2-t1)<mintime c = convn(a,b); k = k + 1; t2 = cputime; end
            K = (t2-t1)/k/prod(siza)/prod(sizb)

        case 3 %%% fft (one dimensional) [[empirically K = 1e-7]] [SCT:blueprint K=2e-8]
            ...
            t1 = cputime;  t2 = t1;  k = 0;
            while (t2-t1) < mintime F = fft(vec); k = k + 1; t2 = cputime; end
            K = (t2-t1) / k / n / log(n);
    end    
