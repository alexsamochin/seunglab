...
function varargout = montage2( IS, showlines, extrainfo, clim, mm, nn, labels )
    ...
    nd = ndims(IS);  % wierd bug in old version of matlab 6.0
    ...
    for i=0:mm-1, for j=0:nn-1,
        k = j+i*nn+1; if( k>siz(4)) i=mm; j=nn; break; end;
        I(rows+i*siz(1),cols+j*siz(2),:) = IS(:,:,:,k); 
    end; end;
    ...
    if( showlines )
        ...
        for i=1:mm-1 height = i*siz(1)+.5; line([.5,montage_width],[height,height]); end
        for i=1:nn-1 width = i*siz(2)+.5; line([width,width],[.5,montage_height]); end
    end

    %%% plot text labels
    textalign = { 'VerticalAlignment','bottom','HorizontalAlignment','left'};
    if( ~isempty(labels) )
        count=1; for i=1:mm, for j=1:nn,
            if( count<=siz(4))
    ...
    for i=mms-1:mm-1, for j=nns-1:nn-1,
