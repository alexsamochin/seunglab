function [h5outputfile] = DownsampleComponentsParallel(h5inputfile, h5inputpath, h5outputfile, h5outputpath, begin_coords, end_coords, desired_head, fac)
...
if(labindex==head)
	...
	create_hdf5_file(h5outputfile, h5outputpath, ceil(input_size(1:3)./2), blksz, blksz, 'int');
	...
	while(keep_going)
		...
		if(tag==1)
			...
			sample_output.new_comp_block= int32(DownsampleComponents(sample_pack.comp_block, sample_pack.fac));
			sample_output.begin_coords=floor(sample_pack.begin_coords./2)+1;
			sample_output.end_coords=sample_output.begin_coords+size(sample_output.new_comp_block)-1;
...
function [] = wait_and_process()
	...
	save /tmp/writetmp sample_output
